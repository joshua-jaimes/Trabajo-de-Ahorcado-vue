...
<img 
  class="ahorcado-img"
  :src="`/ahorcado/${imagenes[intentos]}.png`"
  alt="Ahorcado"
/>
...
const imagenesPorNivel = {
  Facil:  [-2, -1, 0, 1, 2, 3, 4, 5, 6],
  Medio:  [0, 1, 2, 3, 4, 5, 6],
  Dificil:[0, 2, 3, 4, 5, 6]   
};

let imagenes = imagenesPorNivel[nivel];

let inicioImg = imagenesPorNivel[nivel].inicio;
let finImg = imagenesPorNivel[nivel].fin;
...
const maxIntentos = intentosPorNivel[nivel] || 6;
...
function guardarPuntaje(tiempoFinalSegundos, tiempoTexto) {
  ...
  let nivelUrl = route.query.nivel || "???";
  let nivel = nivelUrl
    .replace("Fácil", "Facil")
    .replace("Difícil", "Dificil")
    .replace("Medio", "Medio");
